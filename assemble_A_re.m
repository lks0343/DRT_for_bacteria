...
function out_A_re = assemble_A_re(freq,freq_anal,epsilon,rbf_type,data_type)
...
%   the define the A_re output matrix
    out_A_re_temp = zeros(N_freq,N_anal);
    out_A_re = zeros(N_freq, N_anal+3);
...
%   else we compute all terms with brute force
    if toeplitz_trick && ~strcmp(rbf_type,'Piecewise linear')
        ...
        for iter_freq_n = 1: N_freq
            ...
            freq_m = freq_anal(1);
            C(iter_freq_n, 1) = g_i(freq_n, freq_m, epsilon, rbf_type,data_type);
        ...
        for iter_freq_m = 1: N_anal
            ...
            R(1, iter_freq_m) = g_i(freq_n, freq_m, epsilon, rbf_type,data_type);
        ...
        for iter_freq_n = 1: N_freq
            ...
            for iter_freq_m = 1: N_anal
                ...
                if strcmp(rbf_type,'Piecewise linear')
                    ...
                    out_A_re_temp(iter_freq_n, iter_freq_m) = g_i(freq_n, freq_m, epsilon, rbf_type, data_type);
